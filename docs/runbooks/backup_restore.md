...
2. Restore the database and media recovery set, configure a non-production application instance,
   then run `deploy/database.sh migrate` only if the application commit is newer than the snapshot.
3. Run `php artisan about`, `deploy/database.sh status`, `php artisan api:export-contract --check`,
