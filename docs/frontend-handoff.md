...
## Shipped: 2026-09-07 — Media delivery: longer-lived URLs, ETags, and real dimensions

No response shape changed. Three behaviour changes worth knowing about on the client.

**Signed media URLs now live an hour, not twenty minutes.** `social.media_url_ttl_seconds`
went 1200 → 3600. These URLs are reauthorized on *every* use — the signature only pins the
viewer and the media id — so a longer life costs nothing and fixes the case where an app
returning from the background found every image URL on a still-displayed feed page expired.
The presigned object-store URL behind the redirect went 120s → 300s, because two minutes could
genuinely expire mid-transfer for a large original on a weak uplink.

**Delivery responses now carry a strong `ETag`** (`"pm-{id}-{variant}-{updated_at}"`), and a
matching `If-None-Match` is answered `304` before any presigned URL is minted. Coil is
configured with `respectCacheHeaders(false)` and serves from its own disk cache without a
request at all, so **this changes nothing for the Android app today** — it is for browsers and
any CDN tier placed in front of the origin. Noted so nobody wires up conditional requests
expecting a win that is already being had a cheaper way.

**`width` and `height` now survive the direct-to-R2 upload path.** They were being read off the
object at commit time and then dropped before reaching `post_media`, so any post created through
`upload_ids` would have arrived with both null. Nothing in production had hit this yet — that
path has not been used — but it would have started the day it was switched on.

Two consequences for the client:

- **Treat `width`/`height` as nullable.** The OpenAPI contract has always typed them
  `["integer","null"]`, and legacy rows can still be null. A non-null Kotlin `Int` here means a
  single null takes down the *whole feed page* at Moshi parse time, not one card.
- They are reliable enough now to reserve the card's height from the aspect ratio before the
  bitmap decodes, which is what stops the feed reflowing as images land.

## Shipped: 2026-09-07 — `thumbhash` on post media (instant placeholders)

**New field on every `PostMedia` object: `thumbhash`.** A base64 [ThumbHash](https://evanw.github.io/thumbhash/)
— about 25 bytes decoded — that expands into a small, blurred, colour-accurate impression of the
image. It ships inside the feed JSON, so it costs no extra request and is available the moment the
response lands.
...
"media": [
  {
    "id": 1,
    "position": 0,
    "url": "https://.../thumb.webp",
    "original_url": "https://.../original.jpg",
    "thumbhash": "1fsDBwBoeFmHiHeAd3iIiHh5hZCpCJkK",
    "width": 1080,
    "height": 1920,
    "status": "ready",
    "alt_text": null,
...
**This is the field that removes the blank frame.** Decode it, draw it immediately, and let the
real image cross-fade in over the top. Combined with `width`/`height` reserving the card's height,
a feed should never show an empty box or reflow as pictures arrive.

**It is nullable, in two distinct cases, and both are normal:**

- `status` is `processing` — the hash is computed by the same job that makes the thumbnail, so a
  freshly created post has `url` pointing at the original and `thumbhash` still null.
- The media predates this field. The backfill (`php artisan media:backfill-placeholders`) walks
  existing rows, but treat null as "no placeholder available" indefinitely rather than assuming it
  will be filled.

Fall back to whatever you render today when it is null; do not block on it.

**Decoding:** there is no official Kotlin package. Port `thumbHashToRGBA` from the reference
implementation (MIT) and decode to a small `Bitmap`. Two things worth knowing from doing the
encoder side here:

- **Cross-check your port against the reference, not against this backend.** The vectors in
  `tests/Fixtures/thumbhash-reference-vectors.json` were generated by running the reference
  implementation itself and are a good shared truth for both ports. Two implementations that agree
  with each other but not the format is the failure mode that surfaces only as wrong colours.
- Cache the decoded drawables. Decoding is sub-millisecond, but it happens on the main thread in
  `onBindViewHolder` on every scroll frame, and allocating a bitmap per bind is what jank is made
  of.

## Shipped: 2026-09-07 — `per_page` on the following feed

`GET /v1/feed` and `GET /v1/feed/following` now accept **`per_page`** (1–30), matching
`/v1/feed/for-you`, which already did. Omitting it keeps the previous default of 15, so nothing
changes for a client that does not send it.

Worth knowing why: the Android `PagingConfig` is built around a page size of 20, while these
endpoints hardcoded 15 — so Paging's prefetch arithmetic was working off a number it never
actually received. Send the same value you configure Paging with.

Unchanged: the **first** page of `/feed` can still return more items than requested, because
discovery posts are spliced in at fixed positions (`social.trending.discovery_positions`). Do not
assert `page.size == pageSize` anywhere; Paging handles variable page sizes fine.

## Shipped: 2026-09-07 — Hybrid CDN for public post media (off by default)

**No response shape changed, and nothing to do on the client.** `url` and `original_url` are still
strings; what can change is where they point.

When `SOCIAL_PUBLIC_CDN_ENABLED` is on, media belonging to a **public** post is copied to a
separate public bucket and its URLs become plain CDN links:

```
https://cdn.akukas.ly/<random-32-char-token>/o.jpg
```

One round trip instead of two, edge-cached, and genuinely cacheable by the client. Everything else
— private accounts, private saves, archived and deleted posts — keeps today's signed URL that is
reauthorized on every request.

**Why the client needs no change:** `stableCacheKey()` returns null for a URL with no signing
parameters, so Coil keys on the full URL, which is stable and correct. That is already pinned by a
test so nobody adds blanket query-stripping later.

**The trade, stated plainly.** A CDN URL has no per-viewer check. Someone who obtained one before
being blocked keeps access to those bytes until the token is rotated. The exposure is narrow — feed
and profile queries already exclude blocked authors, so a blocked viewer is never *handed* the URL
— but it is a real behaviour change from the authorized path, not an oversight.

Anything that stops a post being public (archive, delete, purge, going private, deactivation,
moderation) deletes the objects and **rotates the token**, so every URL ever issued for that media
dies. `media:reconcile-public --fix` runs hourly as the net under those triggers.

**Residual, worth knowing:** once bytes have been public and cached at an edge, "unpublish" is
...
## Screenshot usage analytics — 2026-09-20

Deploy the additive screenshot analytics migration/backend before the Android release. Existing clients and requests without `capture_id` remain supported. No historical funnel backfill is performed. The admin page is `/screenshot-analytics`, protected by `viewTelemetry`, with a default 30-day UTC detection cohort, lifetime counts, daily trends, user/installation filters and current library snapshots.
...
A capture may first arrive as a successful operation, because diagnostic uploads are asynchronous. Its authenticated installation claims the ID; later detection fills in the original detection-session user without assigning it to the device's current account. Capture-stage uniqueness survives raw telemetry pruning. Dedicated records cascade on installation deletion and anonymize account references on hard account deletion, following telemetry's foreign-key policy.
